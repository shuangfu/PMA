...
class IndexHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        print("[",datetime.datetime.now(),"] ","IP:%s has connect." % (self.request.remote_ip))
...
class AddProject(tornado.web.RequestHandler):
    def post(self):
        ...
        conn = pymysql.connect(
            host='localhost', user='root', password='lyric', database='lyric_PMA', charset='utf8', port=3309)
        cur = conn.cursor()
        sqlStr = """
        ...
        cur.close()
        conn.close()
        print("[",datetime.datetime.now(),"] ",getData["realName"],"has added new project:",projectName)
...
class LoginHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        cur = conn.cursor()
        data = cur.fetchmany(
            cur.execute("select * from user where un='%s'" % (username)))
        # print(data)
        ...
        cur.close()
        conn.close()
        print("[",datetime.datetime.now(),"] ",realName, " has login")
...
class ChangeStageInfo(tornado.web.RequestHandler):
    def get(self, *args, **kwargs):
        ...
        conn = pymysql.connect(
            host='localhost', user='root', password='lyric', database='lyric_PMA', charset='utf8', port=3309)
        cur = conn.cursor()
        sqlStr = "UPDATE stage SET status='COMPLETED' WHERE stageID = '%s'" % (
            stageID)
        # print("sql:", sqlStr)
        cur.execute(sqlStr)
        conn.commit()
        sqlStr = "UPDATE stage SET actualEndTime = now() WHERE stageID = '%s'" % (
            stageID)
        # print("sql:", sqlStr)
        cur.execute(sqlStr)
        conn.commit()
        ...
        stageName = data[0][0]
        sqlStr = """
            SELECT projectDutyman
            FROM project
            WHERE projectName='%s'
        """ % (projectName)
        data = cur.fetchmany(cur.execute(sqlStr))
        projectDutyman = data[0][0]
        cur.close()
        conn.close()
        print("[",datetime.datetime.now(),"] ",projectDutyman," has changed ",projectName," -> ",stationName," -> ",stageName,"to completed.")
...
class GetStageInfo(tornado.web.RequestHandler):
    def get(self):
        ...
        sqlStr = """
            SELECT projectBelong,stationBelong,stageName
            FROM stage
            WHERE stageID='%s'
         """ % (stageID)
        data = cur.fetchmany(cur.execute(sqlStr))
        projectName = data[0][0]
        stationName = data[0][1]
        stageName = data[0][2]

        cur.close()
        conn.close()
        print("[",datetime.datetime.now(),"] ","somebody has viewed ",projectName," -> ",stationName," -> ",stageName,"to completed.")
...
class GetProjectDelayInfo(tornado.web.RequestHandler):
    def post(self, *args, **kwargs):
        ...
        if data[0][0] == 'manager':
            ...
            print("[",datetime.datetime.now(),"] ","some gm ready to check delayInfo")
        else:
            ...
            sqlStr = """
                ...
                FROM project p, station st, stage sg
                WHERE p.projectDutyMan='%s' and sg.stationBelong=st.stationName and sg.projectBelong=p.projectName and st.projectBelong=p.projectName AND sg.status='RUNNING'
            ...
            print("[",datetime.datetime.now(),"] ","some pm ready to check delayInfo")
        self.write(json.dumps(backdata, ensure_ascii=False))
...
class GetProjectInfo(tornado.web.RequestHandler):
    def post(self):
        ...
        print("[",datetime.datetime.now(),"] ","somebody ready to check details of ",projectName)
        cur.close()
        conn.close()
